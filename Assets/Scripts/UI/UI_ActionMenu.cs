...
public class UI_ActionMenu : MonoBehaviour
{
    ...
    public void Item()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            {
                if(unitInventory[i].GetType() == typeof(HealingItem))
                    UpdateItemSlot(i, unitInventory[i].name, ((HealingItem)unitInventory[i]).currentUses, ((HealingItem)unitInventory[i]).maxUses);
                else
